...
async def update_list_item(
    ...
    Args:
        list_id: The ID of the Slack List (format: F1234ABCD)
        item_id: The ID of the item to update (format: Rec1234ABCD)
        fields: JSON string of fields to update in format:
                [{"column_id": "Col123", "type": "text", "value": "new value"},
                 {"column_id": "Col456", "type": "select", "value": ["OptABC123"]}]
                
                Supported types: text, number, date, select, user, checkbox
    ...
    try:
        ...
        # Parse fields JSON
        try:
            field_updates = json.loads(fields) if isinstance(fields, str) else fields
        except json.JSONDecodeError as e:
...
if __name__ == "__main__":
    ...
    logger.info("Available tools: create_list_item, create_multiple_list_items, get_list_items, filter_list_items, export_list_items, update_list_item")
    
    # Run the MCP server
    mcp.run(transport="stdio")


# Add this to SlackListsClient class - paste manually after get_all_list_items method
